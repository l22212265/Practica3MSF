"""
Práctica 5: Sistema musculoesqueletico
...
fg1 = plt.figure()
plt.plot(t,u2,'-',linewidth=1,color=clr1,label='Fs(t)')
...
PI=controlador(0.0252263601703704,35973.8958893503)
...
plt.plot(t,Fs1,'-',linewidth=1,color=clr2,label='Fs(t)Control')
plt.plot(t,Fs2,'-',linewidth=1,color=clr3,label='Fs(t)Caso')
plt.plot(t,Fs3,'--',linewidth=1,color=clr4,label='Fs(t)Tratamiento')
plt.grid(False)
plt.xlim(0,10); plt.xticks(np.arange(0,11,1))
plt.ylim(-0.1,1.1); plt.yticks(np.arange(-0.1,1.2,0.1))
plt.xlabel('t[s]')
